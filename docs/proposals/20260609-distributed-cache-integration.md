...
**KV cache scope for disaggregated inference:** For `MultiRoleInference`, KV cache must be shared across prefill and decode roles (which are separate InferenceSets). The MultiRoleInference controller propagates consistent `kvCache` config to both role InferenceSets, ensuring they connect to the same KV cache backend. This is handled in controller logic, not by changing the API shape.
