...
func (c *WorkspaceReconciler) addOrUpdateWorkspace(ctx context.Context, wObj *kaitov1beta1.Workspace) (reconcile.Result, error) {
	...
	// TODO(distributed-cache): Integrate ModelMirror with cache warming. When cache is configured,
	// set a provider-specific StorageClass on the ModelMirror PVC so downloads write-through to
	// cache. In Required mode, also gate on provider.IsReady() after ModelMirror reaches Ready.
